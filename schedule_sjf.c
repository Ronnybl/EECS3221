...
#include "task.h"
#include "cpu.h"
//Shortest Job First process scheduling algorithm (Project 2)
...
Task *pickNextTask(){
    ...
    //Unless changed, the current shortest job is the first one we have
    Task *shortJob = phead->task;
    //Run across the entire linked list until the end is reached while performing the following check
    while (curr->next != NULL){
        //If the current task's burst is smaller than the currently recorded shortest job's burst, then swap
        if ((curr->task->burst) < (shortJob->burst)){
            shortJob = curr->task;
        }
